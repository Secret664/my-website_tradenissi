...
<head>
  ...
  <style>
    /* ライトボックス（チャート拡大） */
    /* ※ style.css には記述しない。このファイル内に必ず埋め込む */
    .chart-item img {
      cursor: zoom-in;
...
<body>
  ...
  <!-- 2. ライトボックスオーバーレイ -->
  <!-- 3. スティッキーヘッダー -->
  <!-- 4. .page-wrap（以降すべてのカードを内包） -->
  <!-- 5. スクリプト（</body>直前・必須） -->
</body>
</html>
```

> ⚠️ **`<script>` は必ず `</body>` の直前に配置すること。**
> `<head>` 内や `<body>` 開始直後に置くと DOM が未構築のため
> クリックイベントが一切登録されず、拡大機能が動作しない。
...
### 2. ライトボックスオーバーレイ（固定・毎回出力）

隠しデータ要素の直後、ヘッダーの直前に配置する。

```html
<div id="lightbox" role="dialog" aria-modal="true" aria-label="画像拡大">
...
**ルール**
- `id` は `lightbox` / `lightbox-close` / `lightbox-img` / `lightbox-caption` で固定
- 内容・属性を変更しない
...
- チャート画像はクリックでライトボックス拡大される（スクリプトが自動処理）

---

### 11. ライトボックス スクリプト（固定・毎回出力）

**`</body>` の直前に必ず配置すること。** `<head>` 内や `<body>` 開始直後への配置は禁止。

```html
  <script>
    // チャート画像クリックでライトボックスを開く
...
> ⚠️ スクリプトの配置を `<head>` 内にすると、DOMが未構築のため
> `querySelectorAll` が画像要素を取得できず、拡大が一切動作しない。
> **必ず `</body>` 直前に置くこと。**
...
| 順番 | 要素 | 備考 |
|------|------|------|
| — | `<head>` 内 `<style>` | ライトボックスCSSを埋め込む |
| 0 | 隠しデータ要素 | `class="trade-data"` |
| 0 | ライトボックスオーバーレイ | `id="lightbox"` |
| — | スティッキーヘッダー | `<header>` |
...
- **ライトボックス `<script>` を `<head>` 内または `<body>` 直後に配置しない**
...
- [ ] カード出力順序が仕様通りになっている
- [ ] ライトボックスCSSが `<head>` 内 `<style>` に埋め込まれている
- [ ] ライトボックスオーバーレイ（`id="lightbox"`）が隠しデータ直後に配置されている
- [ ] ライトボックス `<script>` が `</body>` 直前に配置されている
